...
def forward_propogation(X, keep_prob):
	...
	with tf.name_scope('CONV1D'):
		W_conv1 = tf.get_variable(name = 'W_conv1', shape = [5, 5, 3, 8], 
		...
		A_conv1_pool = tf.nn.max_pool(A_conv1, ksize = [1, 2, 2, 1], strides = [1, 2, 2, 1], 
			padding = 'SAME', name = 'A_conv1_pool')

	with tf.name_scope('CONV2D'):
		W_conv2 = tf.get_variable(name = 'W_conv2', shape = [5, 5, 8, 16], 
			initializer = tf.contrib.layers.xavier_initializer(seed = 0))
		...
		A_conv2_pool = tf.nn.max_pool(A_conv2, ksize = [1, 2, 2, 1], strides = [1, 2, 2, 1],
...
def test(prediction, y):
	accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(prediction), tf.argmax(y)), tf.float32))
	return accuracy
...
if __name__ == '__main__':
	...
	with tf.Session() as sess:
		...
		for epoch in range(21):
			sess.run(train_step, feed_dict = {X: train_set_x, y: train_set_y_onehot, keep_prob: 0.7})
			acc = sess.run(accuracy, feed_dict = {X: test_set_x, y: test_set_y_onehot, keep_prob: 1})

			print("Iteration %d, accuracy: %f" %(epoch, acc))
